...
#if defined(NDEBUG)
# error "Exocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x26e8a572ed43f51c7bba635ccfd9b83425d6401390f315a8fc3d27c840d01dac");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Exocoin: starting difficulty is 1 / 2^12
...
{
    // Exocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Exocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Exocoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Exocoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Exocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Exocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x26e8a572ed43f51c7bba635ccfd9b83425d6401390f315a8fc3d27c840d01dac");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Exocoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1516265103;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1374434;
        ...
        assert(block.hashMerkleRoot == uint256("0x1d3a6dfbae1a66f8698d083e9bd407363d582f7f213f1f7e259262be2f24aa72"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Exocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ExocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ExocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ExocoinMiner(CWallet *pwallet)
{
    printf("ExocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("exocoin-miner");
    ...
    try { loop {
        ...
        printf("Running ExocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ExocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ExocoinMiner, pwallet));
}
